#include<iostream>
#include<string>
#include<iomanip>
#include<bit>
#include<cassert>

using namespace std;

constexpr int Nk = 4, Nb = 4, Nr = 10;
constexpr int kBlockSize = 16;

using uchar = uint8_t;
using uint = uint32_t;

static const uchar Sbox[] = {
        
};

static const uchar InvSbox[] = {
        
};

static const uint Rcon[] = {
        0x00000000,
        0x01000000,
        0x02000000,
        0x04000000,
        0x08000000,
        0x10000000,
        0x20000000,
        0x40000000,
        0x80000000,
        0x1b000000,
        0x36000000
};

uint SwapToBe(uint x) {
    if constexpr (endian::native == endian::little)
        x = ((x & 0xff) << 24) | ((x & 0xff00) << 8) | ((x & 0xff0000) >> 8) | ((x & 0xff000000) >> 24);
    return x;
}

uint Sum(uint op1, uint op2) {
    return op1 ^ op2;
}

uchar Mult(uchar op1, uchar op2) {
    const uint p = 0b100011011; // x^8 + x^4 + x^3 + x + 1

    uint result = 0;
    for (int i = 0; i < 8; i++)
        if (op1 & (1 << i))
            result = Sum(result, op2 << i);

    while (result >= 256) {
        int d = 31 - __lzcnt(result);
        result = Sum(result, p << (d - 8));
    }

    return result;
}

void SubBytes(uchar state[Nk][Nb]) {
    for (int i = 0; i < Nk; i++)
        for (int j = 0; j < Nb; j++)
            state[i][j] = Sbox[state[i][j]];
}

void InvSubBytes(uchar state[Nk][Nb]) {
    for (int i = 0; i < Nk; i++)
        for (int j = 0; j < Nb; j++)
            state[i][j] = InvSbox[state[i][j]];
}

void MixColumns(uchar state[Nk][Nb]) {
    // Матрица, на которую умножаем:
    //    {2, 3, 1, 1}
    //    {1, 2, 3, 1}
    //    {1, 1, 2, 3}
    //    {3, 1, 1, 2}

    for (int i = 0; i < Nb; i++) {
        int a = state[0][i];
        int b = state[1][i];
        int c = state[2][i];
        int d = state[3][i];
        state[0][i] = Sum(Sum(Sum(Mult(2, a), Mult(3, b)), c), d);
        state[1][i] = Sum(Sum(Sum(a, Mult(2, b)), Mult(3, c)), d);
        state[2][i] = Sum(Sum(Sum(a, b), Mult(2, c)), Mult(3, d));
        state[3][i] = Sum(Sum(Sum(Mult(3, a), b), c), Mult(2, d));
    }
}

void InvMixColumns(uchar state[Nk][Nb]) {
    // Матрица, на которую умножаем:
    //    {0x0e, 0x0b, 0x0d, 0x09}
    //    {0x09, 0x0e, 0x0b, 0x0d}
    //    {0x0d, 0x09, 0x0e, 0x0b}
    //    {0x0b, 0x0d, 0x09, 0x0e}

    for (int i = 0; i < Nb; i++) {
        int a = state[0][i];
        int b = state[1][i];
        int c = state[2][i];
        int d = state[3][i];
        state[0][i] = Sum(Sum(Sum(Mult(0x0e, a), Mult(0x0b, b)), Mult(0x0d, c)), Mult(0x09, d));
        state[1][i] = Sum(Sum(Sum(Mult(0x09, a), Mult(0x0e, b)), Mult(0x0b, c)), Mult(0x0d, d));
        state[2][i] = Sum(Sum(Sum(Mult(0x0d, a), Mult(0x09, b)), Mult(0x0e, c)), Mult(0x0b, d));
        state[3][i] = Sum(Sum(Sum(Mult(0x0b, a), Mult(0x0d, b)), Mult(0x09, c)), Mult(0x0e, d));
    }
}

void AddRoundKey(uchar st[Nk][Nb], const uint* r) {
    uint temp_arr[Nb];
    for (int i = 0; i < Nb; i++)
        temp_arr[i] = SwapToBe(r[i]);


    uchar* round = reinterpret_cast<uchar*>(temp_arr);
    for (int i = 0; i < Nb; i++) {
        st[0][i] ^= round[i * 4];
        st[1][i] ^= round[i * 4 + 1];
        st[2][i] ^= round[i * 4 + 2];
        st[3][i] ^= round[i * 4 + 3];
    }
}

uint SubWord(uint x) {
    uchar* x_as_bytes = reinterpret_cast<uchar*>(&x);
    for (int i = 0; i < 4; i++)
        x_as_bytes[i] = Sbox[x_as_bytes[i]];
    return x;
}

void RotWord(uchar x[4]) {
    for (int i = 0; i + 1 < 4; i++)
        swap(x[i], x[i + 1]);
}

void InvRotWord(uchar x[4]) {
    for (int i = 3; i - 1 >= 0; i--)
        swap(x[i], x[i - 1]);
}

void ShiftRows(uchar st[Nk][Nb]) {
    RotWord(st[1]);
    RotWord(st[2]); RotWord(st[2]);
    RotWord(st[3]); RotWord(st[3]); RotWord(st[3]);
}

void InvShiftRows(uchar st[Nk][Nb]) {
    InvRotWord(st[1]);
    InvRotWord(st[2]); InvRotWord(st[2]);
    InvRotWord(st[3]); InvRotWord(st[3]); InvRotWord(st[3]);
}

void KeyExpansion(const uchar* key, uint w[Nb * (Nr + 1)]) {
    for (int i = 0; i < Nk; i++)
        w[i] = SwapToBe(*reinterpret_cast<const uint*>(&key[i * 4]));

    for (int i = 4; i < Nb * (Nr + 1); i++) {
        uint temp = w[i - 1];
        if (i % Nk == 0)
            temp = SubWord(temp << 8 | ((temp & 0xff000000) >> 24)) ^ Rcon[i / Nk];
        else if (Nk > 6 && i % Nk == 4)
            temp = SubWord(temp);
        w[i] = w[i - Nk] ^ temp;
    }
}

void EncodeBlock(const uchar* in, uchar* out, const uint w[Nb * (Nr + 1)]) {
    uchar state[Nk][Nb];

    for (int r = 0; r < Nk; r++)
        for (int c = 0; c < Nb; c++)
            state[r][c] = in[r + 4 * c];

    AddRoundKey(state, &w[0]);
    for (int round = 1; round < Nr; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, &w[round * Nb]);
    }

    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, &w[Nr * Nb]);

    for (int r = 0; r < Nk; r++)
        for (int c = 0; c < Nb; c++)
            out[r + 4 * c] = state[r][c];
}

void DecodeBlock(const uchar* in, uchar* out, const uint w[Nb * (Nr + 1)]) {
    uchar state[Nk][Nb];

    for (int r = 0; r < Nk; r++)
        for (int c = 0; c < Nb; c++)
            state[r][c] = in[r + 4 * c];

    AddRoundKey(state, &w[Nr * Nb]);

    for (int round = Nr - 1; round > 0; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, &w[round * Nb]);
        InvMixColumns(state);
    }

    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, &w[0]);

    for (int r = 0; r < Nk; r++)
        for (int c = 0; c < Nb; c++)
            out[r + 4 * c] = state[r][c];
}

string Encode(string_view text, const uchar key[kBlockSize]) {
    uchar* raw_result = new uchar[(text.size() / (kBlockSize + 1) + 1) * kBlockSize + 1];
    raw_result[(text.size() / (kBlockSize + 1) + 1) * kBlockSize] = 0;
    uint w[Nb * (Nr + 1)];
    KeyExpansion(key, w);

    int i;
    for (i = 0; i < text.size() && (i + 15) < text.size(); i += kBlockSize)
        EncodeBlock(reinterpret_cast<const uchar*>(text.data()) + i, raw_result + i, w);

    if (i != text.size()) {
        uchar temp[kBlockSize] = {}; 

        for (int j = (text.size() / kBlockSize) * kBlockSize; j < text.size(); j++)
            temp[j % kBlockSize] = text[j];

        EncodeBlock(temp, raw_result + i, w);
    }

    string result(reinterpret_cast<char*>(raw_result));
    delete[] raw_result;

    return result;
}

string Decode(string_view crypt_text, const uchar key[kBlockSize]) {
    uchar* raw_result = new uchar[(crypt_text.size() / (kBlockSize + 1) + 1) * kBlockSize + 1];
    raw_result[(crypt_text.size() / (kBlockSize + 1) + 1) * kBlockSize] = 0;
    uint w[Nb * (Nr + 1)];
    KeyExpansion(key, w);

    int i;
    for (i = 0; i < crypt_text.size() && (i + 15) < crypt_text.size(); i += kBlockSize)
        DecodeBlock(reinterpret_cast<const uchar*>(crypt_text.data()) + i, raw_result + i, w);

    if (i != crypt_text.size()) {
        uchar temp[kBlockSize] = {}; // Такая инициализация заполняет массив нулями

        for (int j = (crypt_text.size() / kBlockSize) * kBlockSize; j < crypt_text.size(); j++)
            temp[j % kBlockSize] = crypt_text[j];

        DecodeBlock(temp, raw_result + i, w);
    }

    string result(reinterpret_cast<char*>(raw_result));
    delete[] raw_result;

    return result;
}

void printTextToHex(string_view sv) {
    for (int i = 0; i < sv.size(); i++)
        cout << hex << "0x" << ((uint)sv[i] & 0xff) << " ";
    cout << endl;
}


int main() {

    string rawText, rawKey;
    setlocale(LC_ALL, "Russian");

    cout << "Введите текст и ключ: " << endl;
    getline(cin, rawText);
    getline(cin, rawKey);
    
    int i = 0;
    uchar key[kBlockSize];
    for (; i < kBlockSize; i++) {
        if (i < rawKey.size()) {
            key[i] = rawKey[i];
        }
        else {
            key[i] = 0;
        }
    }

    cout << "Введённый текст:" << endl;
    cout  << rawText << endl;
    cout << "16-тиричный: ";
    printTextToHex(rawText);
    cout << endl;

    string encText = Encode(rawText, key);
    cout << "Шифр:" << endl;
    cout << encText << endl;
    cout << "16-тиричный: ";
    printTextToHex(encText);
    cout << endl;

    string decText = Decode(encText, key);
    cout << "Расшифрованный текст:" << endl;
    cout << decText << endl;
    cout << "16-тиричный: ";
    printTextToHex(decText);
    cout << endl;
}